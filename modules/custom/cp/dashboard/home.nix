...
in {
  ...
  home.activation.cpDashboardTakeover = lib.hm.dag.entryBefore ["checkLinkTargets"] ''
  ...
  #--- アセット配置 + Swift ビルド -------------------------------------------
  home.activation.cpDashboardDeploy = lib.hm.dag.entryAfter ["writeBoundary"] ''
          src="${./dashboard}"
          dst="${root}"
          mkdir -p "$dst/out"
  ...
  home.activation.cpDashboardRestartLive = lib.hm.dag.entryAfter ["cpDashboardDeploy"] ''
  ...
  home.activation.cpDashboardPreLaunchBootout = lib.hm.dag.entryBefore ["setupLaunchAgents"] ''
    for label in \
  ...
  home.activation.cpDashboardDoctor = lib.hm.dag.entryAfter ["cpDashboardRestartLive"] ''
